...
def add_trees(t1, t2):
    ...
    if is_leaf(t1):
      ...
      more_t = branches(t1) if len(branches(t1)) > len(branches(t2)) else branches(t2)
      fewer_t = branches(t1) if len(branches(t1)) < len(branches(t2)) else branches(t2)
      fewer_t = fewer_t + [tree(0) for i in range(len(more_t) - len(fewer_t))]
      return tree(label(t1) + label(t2), [add_trees(b1, b2) for b1, b2 in zip(fewer_t, more_t)])
