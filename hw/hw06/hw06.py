...
class VendingMachine:
    ...
    def vend(self):
        if self.number == 0:
            ...
            if remainder == 0:
                ...
                return 'Here is your {} and ${} change.'.format(self.name, remainder)
    ...
    def restock(self, number):
        self.number += number
        return 'Current {} stock: {}'.format(self.name, self.number)
...
def is_bst(t):
    ...
    elif bst_max(t.branches[0]) > t.label or bst_min(t.branches[1]) < t.label:
        return False
    else:
