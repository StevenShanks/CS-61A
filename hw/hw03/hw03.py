...
def g_iter(n):
    ...
    if n <= 3:
        return n
    iter1, iter2, iter3 = 1, 2, 3
    for i in range(4, n+1):
        iter1, iter2, iter3 = iter2, iter3, iter3+2*iter2+3*iter1
    return iter3
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(max, total):
        if total == 0:
            return 1
        elif total < 2**max:
            return 0
        else:
            return helper(max+1, total) + helper(max, total-2**max)
...
def make_anonymous_factorial():
    ...
    True
    """
    return lambda n: (lambda x, f: 1 if x == 1 else x * f(x-1, f))(n, lambda x, f: 1 if x == 1 else x * f(x-1, f))
