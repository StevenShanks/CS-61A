...
def make_withdraw(balance, password):
    ...
    error_count = 0
    error_list = []
    def withdraw(amount, tried_password):
        nonlocal error_count, error_list, balance
        if error_count == 3:
                return "Too many incorrect attempts. Attempts: {}".format(error_list)
        else:
            if password == tried_password:
                if balance >= amount:
                    balance -= amount
                    return balance
                else:
                ...
                error_count += 1
                error_list.append(tried_password)
...
def remainders_generator(m):
    ...
    11
    """
    def get_generator(m, remainder):
        for i in naturals():
            if i % m == remainder:
                yield i
    for i in range(m):
        yield get_generator(m, i)
