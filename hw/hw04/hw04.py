...
def replace_leaf(t, find_value, replace_value):
    ...
    """
    if is_leaf(t):
        return tree(label(t) if label(t) != find_value else replace_value)
    else:
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert 0 > upper_bound(y) or 0 < lower_bound(y)
...
def quadratic(x, a, b, c):
    ...
    point = -b/(2*a)
    if a > 0:
        if point < u and point > l:
            return interval(f(point), max(f(l), f(u)))
        else:
            return interval(min(f(l), f(u)), max(f(l), f(u)))
    else:
        if point < u and point > l:
            return interval(min(f(l), f(u)), f(point))
        else:
            return interval(min(f(l), f(u)), max(f(l), f(u)))
