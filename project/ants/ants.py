...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        for _ in range(self.min_range):
            copy_place = copy_place.entrance
            if copy_place == beehive:
                return None
        while copy_place != beehive and distance <= self.max_range - self.min_range:
            if copy_place.bees:
                return rANTdom_else_none(copy_place.bees)
            else:
                copy_place = copy_place.entrance
                distance += 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        dmg = self.damage + amount if amount >= self.armor else amount
        bees_lst = self.place.bees[:]
        for bee in bees_lst:
            Insect.reduce_armor(bee, dmg)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.is_watersafe:
            Ant.reduce_armor(insect, insect.armor)
        # END Problem 11
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if not self.true_queen:
            ...
            copy_place = self.place
            while copy_place.exit is not None:
                copy_place = copy_place.exit
                if copy_place.ant is not None:
                    curr_ant = copy_place.ant
                    if isinstance(curr_ant, ContainerAnt):
                        if curr_ant.contained_ant is not None and not curr_ant.contained_ant.doubled:
                            curr_ant.contained_ant.damage *= 2
                            curr_ant.contained_ant.doubled = True
                    if not copy_place.ant.doubled:
                        curr_ant.damage *= 2
                        copy_place.ant.doubled = True
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        Ant.reduce_armor(self, amount)
        if self.true_queen and amount >= 1:
            bees_win()
        # END Problem 13
