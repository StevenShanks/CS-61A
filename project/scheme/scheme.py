...
def eval_all(expressions, env):
    ...
    return scheme_eval(expressions.first, env, expressions.rest is nil) # change this line
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    while expressions.rest != nil:
        if is_false_primitive(scheme_eval(expressions.first, env)):
            return False
        expressions = expressions.rest
    last_expr = scheme_eval(expressions.first, env, expressions.rest)
    if is_false_primitive(last_expr):
        return False
    else:
        return last_expr
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    while expressions != nil:
        expr = scheme_eval(expressions.first, env, expressions.rest is nil)
        if is_true_primitive(expr):
            return expr
        expressions = expressions.rest
    return False
    # END PROBLEM 12
...
def do_define_macro(expressions, env):
    ...
    validate_form(expressions, 2) # Checks that expressions is a list of length at least 2
    target = expressions.first
    if isinstance(target, Pair) and scheme_symbolp(target.first): # no need to defind-macro an atom
        name = target.first
        formals = target.rest
        body = expressions.rest
