...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    diff_ones = abs(player_score % 10 - opponent_score % 10)
    return diff_ones == opponent_score // 10 % 10
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        num_of_dice = strategy0(score0, score1) if who == 0 else strategy1(score1, score0)
        opponent_score = score1 if who == 0 else score0
        score = take_turn(num_of_dice, opponent_score, dice)
        if who == 0:
            if feral_hogs and abs(last_score0 - num_of_dice) == 2:
                score0 += 3
            score0 += score
            last_score0 = score
        else:
            if feral_hogs and abs(last_score1 - num_of_dice) == 2:
                score1 += 3
            score1 += score
            last_score1 = score
        my_score = score0 if who == 0 else score1
        opponent_score = score1 if who == 0 else score0
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    curr_score = score + free_bacon(opponent_score)
...
def final_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if is_swap(curr_score, opponent_score):
        if curr_score < opponent_score:
            if opponent_score - curr_score < cutoff:
                return num_rolls
            else:
                return 0
        else:
